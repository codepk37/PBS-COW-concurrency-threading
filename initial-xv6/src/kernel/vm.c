...
/*cowalloc first determines the legality of the virtual address, 
then determines whether the corresponding pte is marked as a COW page.
 If the flag exists, it configures a new physical page for the 
 child process, copies the content of the parent process, deletes the original
  old mapping, and finally remaps to a new physical page, because the page is 
  no longer a COW page, remember to cancel the setting of PTE_C and make the 
  physical page writable*/
...
uvmcopy(pagetable_t old, pagetable_t new, uint64 sz)
{
  ...
  for(i = 0; i < sz; i += PGSIZE){
    ...
    pa = PTE2PA(*pte);
    flags = PTE_FLAGS(*pte);
    // if((mem = kalloc()) == 0)   just made comment
    //   goto err;
