...
int _rbi(struct proc *p){
   if(p->rbi==25){ //rbi like FLAG
      //return 25 only i.e. default 25 ,or after set_priority() resets 25
      //before returning change -1
      p->rbi=-1;
...

{
  ...
  #ifdef PBS
  //Background conclusion:deque,alpid[]=0 waitx exit ,rtime wtime update_time()clockintr(), sahil=1 in userinit
  //number of times process scheduled->  tot_sche 
  //p->state=RUNNING; p->tot_sche++; // number of times the process has been scheduled
  //lower start time-> ctime
// _rbi() , 
//rbi is just flag 25 or -1,  curr_rbi is calcuted considering calc or defalut or reset
  //function to setpriority made, just code  ifnedf endif defalut ,PBS ,and do logic part
  ...
  {// Avoid deadlock by ensuring that devices can interrupt.
        ...
        { 
          ...
          if (p->state == RUNNABLE)
          {
            if(high_pri==0){
              high_pri=p;
              continue;
            }  //min(p->static_pri+ p->curr_rbi,100) -> dp
            ...
            //same staic + dynaimc
                 ...
                 //same dunamic+static , p-> more time scheduled than other , so schedule it
                 release(&high_pri->lock);
                high_pri=p;
                continue;
            }
                  ...
                  //same dynamic+static , same numbre of time tot_sche scheduled , p-> ctime is eariler
                  release(&high_pri->lock);
                high_pri=p;
                continue;
            }
      ...
      //found first process creted having lowest static+dynamic ,and in ready State
      ...
      if(high_pri!=0 && high_pri->state==RUNNABLE){
          ...
          c->proc=0;
          release(&high_pri->lock);
          //total time process is scheduled***
          // we will have to release the lock (which got acquired right before high_pri_coming was last updated), 
...
void procdump(void)
{
  ...
  for (p = proc; p < &proc[NPROC]; p++)
  {
    ...
    // printf("\n");
    // if(p->static_pri+ p->curr_rbi<100){
    //    printf("%d %d \n",p->static_pri, p->curr_rbi);
    // }
